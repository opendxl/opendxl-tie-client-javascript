...
function TieClient (dxlClient) {
   ...
   * @private
   */
  this._setExternalReputation = function (callback, topic, trustLevel, hashes, providerId, fileType, payload, comment) {
    if ((typeof trustLevel === 'undefined') || (trustLevel === null)) {
      throw new TypeError('Error: TrustLevel was not specified')
    }

    if(!(Object.values(TrustLevel).indexOf(trustLevel)>0)) {
      throw new TypeError('Error: TrustLevel was not a valid entry')
    }

    if(!(Object.values(FileType).indexOf(fileType)>0)) {
