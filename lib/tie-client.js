...
var EVENT_TOPIC_CUSTOM_FILE_REPORT = "/mcafee/event/custom/file/report";
...
function TieClient (dxlClient) {
  ...
  this._setReputation = function (callback, topic, trustLevel, providerId,
    ...
    /**
   * Common helper for setting reputations for files from an
   * external provider.
   * @param {Function} callback - Callback to invoke after setting the
   *   reputation.
   * @param {String} topic - DXL topic to which the set reputation request is
   ...
   * @private
   */
  this._setExternalReputation = function (callback, topic, trustLevel, fileType, providerId,
    hashes, payload, comment) {
    var exist = Object.values(TrustLevel).indexOf(trustLevel)>0;

    if(!exist) {
      throw new TypeError('trustLevel was not a valid entry')
    }

    if ((typeof hashes === 'undefined') || (hashes === null) || (Object.keys(hashes).length === 0)) {
      throw new TypeError('File hashes were not specified')
    }
 ...
 *
 * The OpenDXL Python client invoking this method must have permission to send
 * messages to the `/mcafee/event/custom/file/report` topic, which is
...
TieClient.prototype.setExternalFileReputation =
  function(callback, trustLevel, fileType, hashes, filename, comment) {
    filename = filename || ''
    var payload = {}
    payload.file = {
      attributes: {
    ...
    this._setExternalReputation(callback, EVENT_TOPIC_CUSTOM_FILE_REPORT,
        trustLevel, fileType, FileProvider.EXTERNAL, hashes, payload, comment)
 }
